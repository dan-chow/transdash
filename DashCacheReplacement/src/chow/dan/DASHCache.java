...
public class DASHCache extends LoggableCache {
	...
	public void set(String key, Chunk value) {
		...
		iterator = freqKeysMap.entrySet().iterator();
		while (size + value.getSize() > capacity && iterator.hasNext()) {
			Entry<Integer, LinkedHashSet<String>> entry = iterator.next();
	...
	private boolean hasBetter(Chunk chunk) {
		String oldKey = chunk.index + "-" + chunk.level;

		for (int lvl = chunk.level + 1; lvl <= Chunk.LEVELS; lvl++) {
			String newKey = chunk.index + "-" + lvl;
			if (keyValueMap.containsKey(newKey) && keyFreqMap.get(newKey) >= keyFreqMap.get(oldKey)) {
				return true;
			}
	...
	private boolean hasHigher(Chunk chunk) {
		for (int lvl = chunk.level + 1; lvl <= Chunk.LEVELS; lvl++) {
			String newKey = chunk.index + "-" + lvl;
			if (keyValueMap.containsKey(newKey)) {
				return true;
			}
